...
The coefficients for lm0 are -2208 for the intercept and 7683 for diamonds$carat.
...
# You'll want to type your response to question B(1) here.  Your response should look like:
 ...
 n
```
There are 2000 values in diamonds' dataset.
...
# You'll want to type your response to question B(3) here.  Your response should look like:
 resamples <- diamonds[resample1,]
 resample1_ests <- lm(resamples$price~resamples$carat)
 resample1_ests
 coefficients(resample1_ests)
```
The coefficients are -2182.625 for the intercept and 7607.334 for the resamples$carat.
...
for (b in 1:B) {
rowdata <- diamonds[resampled_values[b,],]
new_model <- lm(rowdata[,2]~rowdata[,1])
resampled_ests[b,] <- coefficients(new_model)
}
head(resampled_ests)
```
...
```{r}
hist(resampled_ests[,2], xlab="bootstrap values of beta-1")
```

7) Calculate the standard deviation of the bootstrap estimates.

```{r}
# sd of bootstrap estimate of beta-1
sd(resampled_ests[,2])
# sd of bootstrap estimate of beta-0
sd(resampled_ests[,1])
```
